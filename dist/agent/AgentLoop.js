...
class AgentLoop {
    static #buildAskUserTool({ elicitationConfig }) {
    ...
    static async start({ query, toolClient, systemPrompt, model, maxRounds, maxTokens, onStatus, onRoundLog, onElicit, elicitationConfig, baseURL, apiKey, llmProvider, answerSchema = null, discovery = false }) {
        if (!query) {
        ...
        Logger.info('AgentLoop', `Tools available: ${clientTools.length}`, clientTools.map((t) => t.name));
        ...
        if (onStatus) {
            onStatus({ status: 'working', round, message: 'Agent loop started' });
        }
        do {
            round++;
            if (round > maxRounds) {
            ...
            const content = [
                ...llmResponse.textBlocks.map((text) => ({ type: 'text', text })),
                ...llmResponse.toolCalls.map((tc) => ({ type: 'tool_use', id: tc.id, name: tc.name, input: tc.input }))
            ];
            ...
            if (submitBlock) {
                ...
                running = false;
                if (onStatus) {
                    onStatus({ status: 'completed', round, message: 'Agent loop finished' });
                }
                return { result };
            }
            ...
            if (!hasToolUse || round > maxRounds) {
                ...
                if (toolCallLog.length === 0 && !hasRequestedSubmit && round <= maxRounds) {
                    ...
                    running = false;
                    if (onStatus) {
                        onStatus({ status: 'completed', round, message: 'Agent needs more info (elicitation)' });
                    }
                    return { result };
                }
                if (!hasRequestedSubmit && toolCallLog.length > 0 && round <= maxRounds) {
                    ...
                    if (onStatus) {
                        onStatus({ status: 'working', round, message: `Requesting structured output via ${answerToolName}` });
                    }
                    continue;
                }
                const duration = Date.now() - startTime;
            ...
            const toolResultPromises = toolUseBlocks
                ...
                try {
                    if (name === askUserToolName && onElicit && elicitationConfig) {
                        const { message: elicitMessage, fields: requestedFields } = input;
                        const schemaProperties = {};
                        (requestedFields || []).forEach((fieldKey) => {
                            const fieldDef = elicitationConfig.fields[fieldKey];
                            if (fieldDef) {
                                const prop = { type: fieldDef.type, title: fieldDef.title };
                                if (fieldDef.format) {
                                ...
                                if (fieldDef.enumNames) {
                                    prop.enumNames = fieldDef.enumNames;
                                }
                        ...
                        elicitationCount++;
                        if (onStatus) {
                            onStatus({ status: 'working', round, message: `Asking user: ${elicitMessage}` });
                        }
                        ...
                        if (elicitResult.action === 'accept' && elicitResult.content) {
                            ...
                            if (elicitationCount >= maxElicitations) {
                                const askUserIndex = allTools.findIndex((t) => t.name === askUserToolName);
                                if (askUserIndex !== -1) {
                                    allTools.splice(askUserIndex, 1);
                                }
                            }
                            return {
                                type: 'tool_result',
